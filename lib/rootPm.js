...
(function () {
    ...
    var rootPm = function(rootPolicyFilename, info) {
        ...
        try {
            ...
            logger.info("Warning! Policy manager could not be loaded");
        }
    ...
    function loadIncludePolicy(pmInstance) {
        ...
        xmlParser.parseString(data, function (err, jsonData) {
            if (!err) {
                ...
                //TODO Handle errors
                logger.error('Root policy file parse error: '+err);
                return;
            }
        });
        ...
        logger.log('Policy dir is '+policyDir);
        while(index1 >= 0) {
    ...
    function loadPmCore(pmInstance, type, filename) {
        logger.log('loadPmCore - '+type+' - '+filename);
        //Check if policy file exists
        if(existsSync(filename)) {
            var crypto, dataHash, data = fs.readFileSync(filename);
            try { crypto = require("crypto"); } catch(e) { logger.error ("Cannot load module 'crypto'"); }
                ...
                if (!err) {
                    ...
                    if (existsSync(filename + ".md5")) {
                        if(fs.readFileSync(filename + ".md5") == dataHash) {
                            ...
                            logger.log("Policy schema already checked");
                        }
                    }
                    ...
                    if (checkNeeded) {
                        if(env.validate(jsonData, schema).errors.length === 0) {
                            ...
                            policyWellFormatted = true;
                            logger.log("Policy has a valid schema");
                        } else {
                            policyWellFormatted = false;
                            logger.log('Policy file not valid: ' + filename);
                        }
                    }
            ...
            logger.log('Policy file does not exist: '+filename);
        }
    }
    function testLoadIncludePolicy(pmInstance, mainPolicy) {
        logger.log('loadIncludePolicy');
        pmInstance.includedPolicyFiles = {};
        logger.log("ROOT POLICY FILE: "+mainPolicy);
    ...
    rootPm.prototype.testRequest = function(request, path){
        ...
        if (!this.pmCore['user'] || !this.pmCore['manufacturer']) {
            ...
            logger.log(this.rootPolicyFile);
            testLoadIncludePolicy(this, mainPolicy);
            logger.log(JSON.stringify(this.includedPolicyFiles));
            loadPmCores(this);
            logger.log(JSON.stringify(this.pmCore));
            logger.log(JSON.stringify(this.rootPolicy));
        }
    ...
    rootPm.prototype.enforceRequest = function(request) {
        if (policyGlobalVersionCounter != this.policyLocalVersionCounter){
            logger.log("Policy Enforcement: Detected version mismatch (Global = " +
        ...
        var res = 1;
        if (!this.pmCore) {
            logger.error("Invalid policy file: request denied")
            return 1;
        }
        if(this.rootPolicy['policy-set']) {
            var date = new Date();
        ...
        //logger.log('rootPm - enforceRequest returning '+res);
        return (res);
    };
    ...
    function testPolicy(pmInstance, policy, request, path) {
        ...
        if(combAlg >= 0) {
            ...
            if(policy['policy-set']) {
                for(var j in policy['policy-set']) {
                    logger.log("POLICY: "+policy['policy-set'][j]);
                    ...
                    if(policy['extfile'][j] == 'app') {
                        ...
                        if(request.widgetInfo && request.widgetInfo.id) {
                            ...
                            if(!pmInstance.pmCore[request.widgetInfo.id]) {
                                ...
                                logger.log('app filename is '+appFilename);
                            ...
                            if(pmInstance.pmCore[request.widgetInfo.id]) {
                                ...
                                logger.log("TEST REQUEST PATH"+ JSON.stringify(path));
                            }
                        }
                        //logger.log('pm for app returned '+resTmp);
                    }
                    else {
                        if(pmInstance.pmCore[policy['extfile'][j]]) {
                            ...
                            logger.log("TEST REQUEST PATH"+ JSON.stringify(path));
                            //logger.log('pm for '+policy['extfile'][j]+' returned '+resTmp);
                        }
                    }
                    res = LUTs[combAlg][res][resTmp];
                }
        ...
        //logger.log('evaluatePolicy returning '+res);
        return(res);
    }
    ...
    function updatePolicyVersion() {
        ...
        logger.log("policyGlobalVersionCounter set to " + policyGlobalVersionCounter);
    };
    ...
    rootPm.prototype.reloadPolicy = function (genericURI) {
        ...
        loadIncludePolicy(this);
        loadPmCores(this);
        logger.log("Policy version was " + this.policyLocalVersionCounter);
        this.policyLocalVersionCounter = policyGlobalVersionCounter;
        logger.log("Policy updated version was " + this.policyLocalVersionCounter);
    };
